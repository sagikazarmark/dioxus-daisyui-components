//! The `schemaform_daisyui` mapping: the `dioxus-field` bindings the renderer package builds from
//! `schemaform-dioxus`'s edit hooks, and the field metadata it derives from a control's
//! presentation. Driven through a capturing renderer and a `VirtualDom`, without markup: what is
//! under test is the binding contract every other `dioxus-field` consumer relies on.
//!
...
use dioxus_daisyui_components::components::schemaform_daisyui::{
